...
curl -s -X POST http://localhost:9000/purge
curl -sH "Content-Type: application/json" http://localhost:9000/fleet -d @fleet.json

if ! [ -e "serviceLocations.json" ]; then 
	wget http://assets.springone2gx2015.s3.amazonaws.com/fleet/serviceLocations.json
fi
curl -s -X POST http://localhost:9001/purge
curl -sH "Content-Type: application/json" http://localhost:9001/bulk/serviceLocations -d @serviceLocations.json

echo "Starting simulator..."
# Start the simulator
curl -s http://localhost:9005/api/cancel > /dev/null
curl -s http://localhost:9005/api/dc > /dev/null
echo "**** Vehicle Fleet Demo is running on http://localhost:8080"
